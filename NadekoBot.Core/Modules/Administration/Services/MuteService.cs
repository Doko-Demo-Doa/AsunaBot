    ...
    public class MuteService : INService
    {
        ...
        public async Task<IRole> GetMuteRole(IGuild guild)
        {
            ...
            if (muteRole == null)
            {
                ...
                try { muteRole = await guild.CreateRoleAsync(muteRoleName, GuildPermissions.None, null, false, null).ConfigureAwait(false); }
                catch
                {
                    //if creations fails,  maybe the name is not correct, find default one, if doesn't work, create default one
                    muteRole = guild.Roles.FirstOrDefault(r => r.Name == muteRoleName) ??
                        await guild.CreateRoleAsync(defaultMuteRoleName, GuildPermissions.None, null, false, null).ConfigureAwait(false);
                }
            }
